...
void setup() {
  ...
  lane4 = new level(800,385,200,20);
...
// So player doesn't move throught first wall
   ...
   if ((Yplayer >= Ywall1 && Xplayer >= Xground2)) {Yplayer = 400;}
...
// moving right and left    
  if (keyCode == LEFT) {Xplayer -= Xspeed; teleportLeft = true;}  
  if (keyCode == RIGHT) {Xplayer += Xspeed; teleportRight = true;}
...
// making the player able to climb
  if ((climbPressed == true) && (Xplayer >= 970)) {climb = true;}
  else climb = false;
