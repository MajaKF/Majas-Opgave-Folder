...
void setup() {
  ...
  lane5 = new level(650,385,75,laneWidtht);
  lane6 = new level(375,385,75,laneWidtht);
  lane7 = new level(175,385,75,laneWidtht);
  lane8 = new level(25,385,75,laneWidtht);
  lane9 = new level(230,165,100,laneWidtht);
  lane10 = new level(650,165,100,laneWidtht);
...
//snapping player back to the ground after jumping    
  ...
  else if ((jump = true) && (Yplayer <= 365) && (Yplayer >= 200)) {Yplayer += 1;}
  else if ((jump = true) && (Yplayer <= 145)) {Yplayer += 1;}
  
//to prevent player from jumping more then once  
  if ((jump == true && Yplayer <= 520 && Yplayer >= 380)) {Yplayer = 570;}
  else if ((jump == true && Yplayer <= 315 && Yplayer >= 300)) {Yplayer = 365;}
  else if ((jump == true && Yplayer <= 95 && Yplayer >= 85)) {Yplayer = 145;}
  else Yplayer -= 0;
...
//so player doesn't move throught first wall
   if ((Xplayer > 970) && (Yplayer > 380) && (Yplayer > 145)) {Xplayer = 970;}
   else Xplayer -= 0;
   
}

void thirdFloor() {
 //so player doesn't move throught third wall
   if ((Xplayer > 970) && (Yplayer > 145)) {Xplayer = 970;}
   else Xplayer -= 0;
...
// making the player able to climb up second wall
  if ((climbPressed == true) && (Xplayer <= 30) && (Yplayer <= 380) && (Yplayer >= 320)) {climb2 = true;}
  else climb2 = false;
  if (climb2 == true) {Yplayer = Yplayer - 250;}
  else if ((climbPressed == true) && (Xplayer <= 30)) {climb2 = false;}
  else if ((climbPressed == true) && (Yplayer >= 380)) {climb = false;}
